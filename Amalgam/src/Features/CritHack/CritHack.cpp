...
void CCritHack::Draw(CTFPlayer* pLocal)
{
	...
	if (tStorage.m_flDamage >= 0.f)
	{
		...
		else if (pWeapon->m_flCritTime() > flTickBase)
		{
			...
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, std::format("Deal {} damage", ceilf(m_flDamageTilFlip)).c_str());
		...
		else
		{
			...
			else
			{
				...
				H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, std::format("Crit in {}{} shot{}", iShots, iShots == 1000 ? "+" : "", iShots == 1 ? "" : "s").c_str());
				//H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, "No crits");
			}
		}

		int iCrits = tStorage.m_iAvailableCrits;
		H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("{}{} / {} crits", iCrits, iCrits == 1000 ? "+" : "", tStorage.m_iPotentialCrits).c_str());
		...
		H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, "Calculating");

	if (m_flDamageTilFlip && iSlot != SLOT_MELEE)
	{
		if (!m_bCritBanned)
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextGood.Value, align, std::format("{} damage", floor(m_flDamageTilFlip)).c_str());
		//else
		//	H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, std::format("Deal {} damage", ceilf(m_flDamageTilFlip)).c_str());
	}
	...
	{
		H::Draw.String(fFont, x, y += nTall * 2, Vars::Menu::Theme::Active.Value, align, std::format("RangedDamage: {}, CritDamage: {}", m_iRangedDamage, m_iCritDamage).c_str());

#ifdef SERVER_CRIT_DATA
		H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("AllDamage: {} ({})", m_iRangedDamage + m_iMeleeDamage, m_iMeleeDamage).c_str());
