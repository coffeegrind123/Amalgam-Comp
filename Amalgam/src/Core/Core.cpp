...
void CCore::Load()
{
	// File-based logging for Core.Load debugging - OUR ENHANCEMENT
	...
	try
	{
		// Initialize safe access handler FIRST - this catches null pointer crashes - OUR ENHANCEMENT
		...
		while (true)
		{
			...
			auto uDereference = uSignature ? *reinterpret_cast<uintptr_t*>(U::Memory.RelToAbs(uSignature)) : 0; // UPSTREAM IMPROVEMENT
			...
			{
				AppendFailText(std::format("Failed to load in time:\n  {:#x} ({:#x})\n  {:#x}", uDereference, uSignature, uintptr_t(hWindow)).c_str()); // UPSTREAM IMPROVEMENT
				return;
			}
		...
		// Initialize SIMD math system for vector operations optimization - OUR ENHANCEMENT
		try {
...
void CCore::Unload()
{
	...
	{
		LogFailText();
		U::SafeAccess.Shutdown(); // OUR ENHANCEMENT
		return;
	}
	...
	F::Visuals.RestoreWorldModulation(); // OUR ENHANCEMENT - moved before third person check
...
>>>>>>> upstream/master
	...
	// Shutdown safe access handler last - OUR ENHANCEMENT
